...
// Runs an ad hoc Place The Flag SG base raid
...
#define PLACES_TO_WIN 	3		// number of flag places needed for victory
...
int PTFRaidOnEnterMap(ENTITY player)
{
...
int PTFRaidOnLeaveMap(ENTITY player)
{
	SetAIGang(player, 0);
...
//			CreateIOPs();
			...
			OnPlayerEnteringMap( PTFRaidOnEnterMap );
			OnPlayerExitingMap( PTFRaidOnLeaveMap );
		END_ENTRY
...
void PTFRaidInit()
{
	SetScriptName( "PTFRaid" );
	SetScriptFunc( PTFRaid );
